...
const { 
    addIssue, deleteIssue, getIssues, editIssue, deleteAllIssues, setIssueVote, setIssueVotingDone, resetVotes 
} = require('./issues')
...
io.on('connection', (socket) => {
    ...
    socket.on('resetVotes', (issueId, callback) => {
        const { issue, error } = resetVotes(issueId)
        if (error) return callback(error)
        io.in(issue.room).emit('issues', getIssues(issue.room))
    })
    ...
    socket.on('deleteAllIssues', (roomId, message) => {
        io.in(roomId).emit('message', { description: `${message}` })
